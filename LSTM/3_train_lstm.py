...
# Define Base Feature Dictionary
base_features = {
    "L": ["L_Mean", "L_Std"],
    "a": ["a_Mean", "a_Std"],
    "b": ["b_Mean", "b_Std"],
    ...
    "Day": ["Day"],
    "Temp": ["Temp"],
    # Add Orange & Yellow keys
    "Orange": ["Mixed_RedGreen_Mean", "Mixed_RedGreen_Std"],
    "Orange_Lab": ["Mixed_L_Mean", "Mixed_a_Mean", "Mixed_b_Mean", 
                   "Mixed_L_Std", "Mixed_a_Std", "Mixed_b_Std"],
    "Orange_HSV": ["Mixed_H_Mean", "Mixed_S_Mean", "Mixed_V_Mean",
                   "Mixed_H_Std", "Mixed_S_Std", "Mixed_V_Std"],

    "Yellow": ["Mixed_RedGreenYellow_Mean", "Mixed_RedGreenYellow_Std"],
    "Yellow_LAB": ["Mixed_Yellow_L_Mean", "Mixed_Yellow_a_Mean", "Mixed_Yellow_b_Mean",
                   "Mixed_Yellow_L_Std", "Mixed_Yellow_a_Std", "Mixed_Yellow_b_Std"],
    "Yellow_HSV": ["Mixed_Yellow_H_Mean", "Mixed_Yellow_S_Mean", "Mixed_Yellow_V_Mean",
                   "Mixed_Yellow_H_Std", "Mixed_Yellow_S_Std", "Mixed_Yellow_V_Std"]
}

def progressive_features(features_dict):
    """
    Create feature combinations, always forcing 'Orange' and 'Yellow' in every combination.
    """
    feature_combinations = {}
    all_keys = list(features_dict.keys())
    
    # Remove 'Orange' and 'Yellow' from the normal loop
    # so we can forcibly add them to each combination
    forced_keys = ["Orange", "Yellow"]  # you can also force 'Orange_Lab', 'Yellow_HSV', etc. if desired
    for key in forced_keys:
        all_keys.remove(key)
    
    # Now combine the rest, but always include forced_keys
    # i ranges from 0 to len(all_keys) so that we can have everything from "just Orange+Yellow"
    # up to "Orange+Yellow + all other keys".
    for i in range(0, len(all_keys) + 1):
        for comb in combinations(all_keys, i):
            comb_list = list(comb) + forced_keys  # add 'Orange' and 'Yellow'
            comb_list = sorted(comb_list)  # optional, just to keep the name consistent
            # Merge columns from each key into one list
            combined_columns = []
            for key in comb_list:
                combined_columns += features_dict[key]
            feature_name = "_".join(comb_list)
            feature_combinations[feature_name] = combined_columns
    
    logging.info(f"Generated {len(feature_combinations)} feature combinations (all include Orange & Yellow).")
    return feature_combinations

def mix_orange(df, red_weight=0.8, green_weight=0.2):
    # Create Orange features
    ...
    mixed_rgb = np.stack([df["Mixed_RedGreen_Mean"], 
                          df["Mixed_RedGreen_Mean"], 
                          np.zeros_like(df["Mixed_RedGreen_Mean"])], axis=1)
    mixed_bgr = np.array(mixed_rgb, dtype=np.uint8)[:, np.newaxis, :]  # BGR format for OpenCV
...
def mix_yellow(df, red_weight=0.5, green_weight=0.5):
    ...
    # Convert Mixed RGB to LAB & HSV
    mixed_rgb = np.stack([df["Mixed_RedGreenYellow_Mean"], 
                          df["Mixed_RedGreenYellow_Mean"], 
                          np.zeros_like(df["Mixed_RedGreenYellow_Mean"])], axis=1)
    mixed_bgr = np.array(mixed_rgb, dtype=np.uint8)[:, np.newaxis, :]
...
# Create all combinations (always including Orange & Yellow)
...
for feature_name, feature_list in feature_sets.items():
    ...
    # Split Data
    X_train, X_test, y_train, y_test = train_test_split(X_reshaped, y_scaled, 
    ...
    # Callbacks
    checkpoint = ModelCheckpoint(f"best_lstm_{feature_name}.h5", 
                                 save_best_only=True, 
                                 monitor='val_loss', 
                                 mode='min')
